...
The full write-ups behind the summaries in the top-level readme.
...
Revision `0001` is the schema the models describe, and `alembic upgrade head` is
the one safe command everywhere. The one-time adoption machinery — stamping
pre-Alembic databases with the baseline in `env.py`, plus a reconciliation
revision that rebuilt the drifted tables — was removed once every real database
had been migrated past it. `alembic check` passes against a fresh database and
an upgraded production one alike, which is the property that keeps the next
autogenerated migration honest.
